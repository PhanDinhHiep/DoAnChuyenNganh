...
<%    int sodh = Integer.parseInt(request.getParameter("shd"));
    Donhang ctdh = Donhang.layCTdonhang(sodh, nv.getSodtnv());
...
<div class="col-lg-9" style="padding-top: 20px; padding-bottom:20px;">
    <form method="post" action="nvgh_thongtinchuagiao.jsp">
        ...
        <table class="table table-striped" style="border: #495057; background-color: white;">
            ...
            <tbody>
                <%
                    ArrayList<Monhang> dsdh = Chitietdonhang.laychitietDonHangGiaohang(sodh, nv.getSodtnv());
                    int i = 1;
                    double tong = 0;
        ...
        <div class="form-group shadow-textarea">
            ...
            <textarea class="form-control z-depth-1" name="lydo" id="ckeditor1" rows="5" placeholder="Nhập lý do"></textarea>
        </div>
        <div class="col-md-6 mb-3">
            <label for="firstName" style="color: red"><h4>Trạng thái đơn hàng(*)</h4></label>
            <select name="tuychon" class="custom-select d-block w-100" title="Chọn...">               

                <option>Đã giao hàng</option>
                <option>Không giao được hàng</option>
            </select>
        </div>
    ...
    <form action="nvgh_dagiao.jsp?shd=<%=sodh%>" method="post">
        <div class="col-md-3 mb-3">
            <button class="col-lg-12 btn btn-primary" type="submit"> Đã giao
            </button>
        </div>
    </form>
</div>
