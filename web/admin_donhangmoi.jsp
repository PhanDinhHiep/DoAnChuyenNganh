...
<div class="col-lg-9" style="padding-top: 20px;">
    <div class="container">
        <h4 style="color: #005cbf">Danh sách đơn hàng mới</h4>
        <table class="table table-striped" style="border: #495057; background-color: white;">
            ...
            <tbody>
                <%                    
                    int trang = 1, dau = 0, cuoi = 0;
                    int tam = 0;
                    if (request.getParameter("trang") != null) {
                        trang = Integer.parseInt(request.getParameter("trang"));
                    }

                    int tong = Donhang.layDonHangMoi().size();
                    if (tong <= 9) {
                        dau = 0;
                        cuoi = tong;
                    } else {
                    ...
                    Vector<Donhang> kq = new Vector();
                    for (int i = dau; i < cuoi; i++) {
                        kq.add(Donhang.layDonHangMoi().get(i));
                    }
                    for (Donhang s : kq) {
                %>
                <tr>         
                    <td class="list-group-item-primary"><a href="ad_chitietdonhang.jsp?shd=<%=s.getSodonhang()%>"><%=s.getSodonhang()%></a></td>
